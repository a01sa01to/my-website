...
const opendataRequest_old = (req, res) => {
    const c400 = () => {
        res.status(400).sendFile(path_1.default.join(new_dirname, 'err', '400.html'));
        return;
    };
    try {
            ...
            if (req.query.filter) {
                ...
                filt.forEach((f) => {
                    switch (f.type) {
                        ...
                        case 'date':
                            if (f.mode === 'from') {
                                fileToJson = fileToJson.filter((data) => new Date(data[f.key]) >= new Date(f.val));
                            }
                            else if (f.mode === 'to') {
                                fileToJson = fileToJson.filter((data) => new Date(data[f.key]) <= new Date(f.val));
                            }
                        ...
                        case 'num':
                            if (f.mode === 'over') {
                                fileToJson = fileToJson.filter((data) => Number(data[f.key]) >= Number(f.val));
                            }
